...
function Subtitle() {
    ...
    useEffect(() => {
        ...
        const fetchRolesAndFireStationInfo = async () => {
            try {
                const roleResponse = await axios.get('http://localhost:8081/api/auth/role', {
                    headers: {
                ...
                if (roleResponse.data.roles.includes('ROLE_FIRETEAM')) {
                    const fireStationResponse = await axios.get('http://localhost:8081/api/firestation/info', {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                    });
